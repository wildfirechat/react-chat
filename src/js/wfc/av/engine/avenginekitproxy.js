...
export class AvEngineKitProxy {
    ...
    showCallUI(conversation, isConference) {
        ...
        if (isElectron()) {
            ...
            let win = window.open(window.location.origin + '?' + type, '_blank', `width=${width},height=${height},left=200,top=200,toolbar=no,menubar=no,resizable=no,location=no,maximizable=no,resizable=no,dialog=yes`);
            if (!win) {
